...
def extract_complete_gaussian_results():
    ...
    for log_file in log_files:
        try:
            ...
            if "GaussNoise" in test_type:
                ...
                results[model_name][test_type]['F1'] = data.get('test_pixel-level F1', 'N/A')
                results[model_name][test_type]['IOU'] = data.get('test_pixel-level IOU', 'N/A')
                results[model_name][test_type]['AUC'] = data.get('test_pixel-level AUC', 'N/A')
    ...
    report_lines = []
    report_lines.append("=== Complete Gaussian Noise Robustness Test Results ===\n")
    report_lines.append("Metrics: F1 Score, IOU, AUC\n\n")
    ...
    for model_name in sorted_models:
        ...
        for test_type in test_types:
            ...
            report_lines.append(f"  F1 Score: {test_data['F1']}")
            report_lines.append(f"  IOU: {test_data['IOU']}")
            report_lines.append(f"  AUC: {test_data['AUC']}")
    ...
    print(f"Successfully extracted complete Gaussian Noise results from {len([f for f in log_files if 'GaussNoise' in f])} log files")
    print(f"Report saved to robust111.txt")
    return len([f for f in log_files if 'GaussNoise' in f])

if __name__ == "__main__":
    extract_complete_gaussian_results()
